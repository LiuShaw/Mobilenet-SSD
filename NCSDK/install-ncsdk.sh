...
if [ "${OS_DISTRO,,}" == "ubuntu" ] || [ $OS_VERSION == 1604 ]; then
	...
	ERR_MSG=$(cat <<- END
		${RED}You are running Raspbian Jessie, which is not supported by NCSDK.
...
$SUDO_PREFIX cp $DIR/mvnc.h $INSTDIR/include
$SUDO_PREFIX cp $DIR/mvnc_deprecated.h $INSTDIR/include
...
# Install python API
$SUDO_PREFIX cp ${DIR}/mvncapi.py $SDK_DIR/api/python/mvnc
$SUDO_PREFIX cp ${DIR}/__init__.py $SDK_DIR/api/python/mvnc
